...
4. Registers into v2 domains:
   ...
   - `chat.message` fidelity → `ctx.session.hook("prompt")` (the
     session-prompt bridge, registered unconditionally when the v1
     `chat.message` hook exists): once-per-admission identity with the
     eventual inbox User messageID
   - `chat.headers` → `ctx.session.hook("model.request")` (unconditional
     when the v1 hook exists): per-provider-request HTTP headers. The context hook records
     ...
     mutation)
   - `session.compaction` → `ctx.session.hook("compaction")`
     (unconditional): strips the plugin's tagged synthetic injections
     from the host's summarization request
   - child-session permission rules → `ctx.session.update({sessionID,
     permissions})` on plugin-managed child `session.created` events
...
Each bridge in step 4 registers into its own domain: the domain transforms
(agent/tool/mcp/command) are independently try/catch-guarded so one failure
cannot disable the rest, while the session hooks (`prompt`, `context`,
`model.request`, `compaction`) register unconditionally on full v2 contexts
— a registration failure fails setup loudly. A
zero-registration load logs a health-check warning.
