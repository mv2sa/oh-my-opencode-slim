...
- `event`: Evaluates managed idle liveness after child-running suppression, atomically reconciles running same-epoch tool operations to interrupted via `controller.store.reconcileIdleOperations`, suppresses valid waits and active children, and emits at most one internal action-required wake per idle spell while surfacing durable read failures.
