...
- **stop-confirmation.ts**: Shared configurable grace for explicit child-idle observations. Missing runtime status is uncertainty-only, and positive parent busy/retry blocks terminal inference and resets the clock. Confirmed durable stop evidence calls `markStopped` and can wake the parent.
