...
3. **Deterministic Evidence Evaluation**:
   ...
   - Evaluate supplied deterministic evidence receipts for relevance, completeness, candidateFingerprint alignment with final candidate, freshness, and status.
...
5. **Exceptions & External Waiver Authority**:
   ...
   - Every waived rule must reference an explicit external authorization (\`repository_waiver\` or \`user_decision\`) with a valid authorizationReference.
...
- \`CONTINUE\`: Work is in progress within scope, but additional execution steps or evidence are required.
- \`CORRECT_DRIFT\`: Implementation has drifted from scope, violated repository governance rules, or produced failing validations. Corrective action required.
...
- \`ACCEPT\`: Deliverables fully satisfy requested outcome, all applicable governance rules are satisfied or have externally authorized justified exceptions, fresh passing final-candidate evidence matches candidateFingerprint, handoff is ready with verification steps, and lifecycle receipts agree.

**Output Format**:
You MUST emit exactly one \`<outcome_review>\` JSON envelope matching the following structure:
...
<outcome_review>
{
  ...
  "candidateFingerprint": "git-sha-or-deliverable-fingerprint",
  "goals": [
    {
      "id": "goal-1",
  ...
  "rules": [
    {
      ...
      "category": "governance",
      "summary": "Summary of applicable rule",
  ...
  "evidence": [
    {
      ...
      "fingerprint": "git-sha-or-deliverable-fingerprint",
  ...
  "exceptions": [
    {
      "ruleId": "rule-1",
      "justification": "Justification for waiver/exception",
      "justified": true,
