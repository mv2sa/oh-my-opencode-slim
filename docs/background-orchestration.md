...
- Wake requires valid host response shapes, parent currently idle, and at least
  ...
  fails closed. **Active children suppress periodic TODO wakes**, but not a
  stopped-child recovery wake (which also does not require incomplete TODOs).
...
- marks a durable cooldown for the failed model matching the parsed reset interval (capped at 5 hours, or 7 days for a monthly-window exhaustion; see [Persistent Model Cooldowns](configuration.md#persistent-model-cooldowns) for the registry path and recovery procedure),
